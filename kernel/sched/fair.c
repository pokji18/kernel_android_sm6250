...
redo:
	 ...
	 *
	 * @prevp is latched before the body runs because a successful
	 * detach_task() takes @p off this list.  Only @p is ever removed
	 * here and the rq lock is held throughout, so @prevp stays valid.
	 *
	...
	for (p = list_last_entry(tasks, struct task_struct, se.group_node);
	     &p->se.group_node != tasks; p = prevp) {
		prevp = list_entry(p->se.group_node.prev,
		...
		if (((cpu_rq(env->src_cpu)->nr_running > 2) ||
			(env->flags & LBF_IGNORE_BIG_TASKS)) &&
			!lb_cost_fits(env, &concession, p, load))
			goto next;
...
next:
	 ...
	 *
	 * @resume is the first unexamined task, and the subset from the
	 * examined block down to the old tail is exactly what this pass
	 * looked at.  Making @resume the new tail is what a rotation does,
	 * so the surviving rejects between the old head and @resume move to
	 * the front together, keeping their relative order, and a later
	 * pass resumes where this one left off.
	 *
	 ...
	 * examined), or nothing was examined at all (@resume is the last
	 * entry, so it is already in back).
	 */
	if (sched_feat(LB_ROTATE_BLOCK) && resume &&
	    resume != tasks && resume->next != tasks)
		list_move_tail(resume, tasks);
